function turnpopup() {
    ...
    switch (m.phase) {
        case 0:
            ...
            if (difficulty > 10) {
                if (difficulty < 40) {
                    findAttackingRaiders();
                    if (
                        raiderSpawnsThatCanAttack.length > 0 &&
                        ((difficultymultiplier *
                            ((getRandomInt(m.spy, 3) ? 1 : 0.5) * 0.0625 * difficulty ** 2.5 +
                                getRandomInt(-10, 5)) >
                            military &&
                            getRandomInt(0, 3 + m.assissin) == 1) ||
                            getRandomInt(0, 4) == 0)
                    ) {
                        popups[0].choosetext();
                ...
                if (wars.length > 0) {
                    ...
                    if (getRandomInt(0, 1) == 0) {
                        ...
                        getRandomInt(0, 1) == 0 ||
                        (military > warpower && getRandomInt(0, 2) != 0)
                    ) {
                        ...
                        warpower / military > 4 ||
                        (warpower / military < 0.25 && !isdivine)
                    ) {
                    ...
                    getRandomInt(
                        ...
                        7 +
                            Math.max(
                                -5,
                                Math.floor(
                                    (reputation / 5 + m.assissin) *
                                        (getRandomInt(7, 14) / 10) *
                                        ((difficultymultiplier *
                    ...
                    getRandomInt(
                        ...
                        Math.max(
                            ...
                            (3 -
                                Math.max(
                                    -7,
                                    Math.min(15, Math.floor((currentpop - population) / 4))
                                )) *
                                Math.min(3 - difficultymultiplier, food / currentpop) +
                                m.rebel +
                                punishamount +
                                (techstats.social_care ? 2 : 0) -
                                outofrange
                        )
                    ) <= 0
                ) {
        ...
        case 1:
            ...
            if (
                getRandomInt(
                    ...
                    Math.max(
                        0,
                        (3 - Math.max(-7, currentpop - population)) *
                            Math.min(3 - difficultymultiplier, food / currentpop) +
                            m.rebel
                    ) +
                        (techstats.social_care ? 2 : 0) -
                        Math.floor(outofrange * 5)
                ) <= 0
            ) {
                popups[1].choosetext();
                displaypopup(1);
...
function enable() {
    ...
    if (turnreturn == true) {
        if (!psettings.nofade) {
            document.getElementById('popup_block_buttons').style.animation =
                'popup_finish linear 1s 1 normal forwards';
        }
        setTimeout(function () {
            document.getElementById('popup_block_buttons').animation = 'none';
...
function next_turn() {
    ...
    xp +=
        Math.floor(Math.max(0, Math.min(1 + Math.ceil(currentpop / 5), food - currentpop)) ** 0.5) *
        10 *
        (1 + techstats.innovation);
    currentpop += Math.max(
        -2 - Math.ceil(currentpop / 5),
        Math.min(1 + Math.ceil(currentpop / 5), food - currentpop)
    );
    for (const d of divisions) {
        d.xp += Math.ceil(trainingXp / d.power) * (1 + modifiers.military);
        while (d.xp > 50 + (100 * division.level) / 2 && d.level < divisionLevelToTitle.length) {
            d.level++;
        }
    }
    ...
    if (difficulty > 20) {
        for (const p of m.marketselections) {
            p.turnmodify();
            if (p.title != 'Mysterious Artifact' && p.title != 'Blueprints') {
                p.stock += getRandomInt(-1, 2);
            }
...
function displayUI(turn = false) {
    ...
    if (m.phase > 1) {
        ...
        document.getElementById('boss_health').style.width =
            Math.max(0, 100 * (m.bhealth / m.totalbhealth)) + '%';
        setTimeout(function () {
            document.getElementById('boss_healthg').style.width =
                document.getElementById('boss_health').style.width;
        }, 1000);
        document.getElementById('boss_health_text').innerHTML =
            'boss: ' + shorten(Math.max(0, m.bhealth)) + '/' + shorten(m.totalbhealth);
        ...
        document.getElementById('boss_health').style.width =
            Math.max(0, 100 * (enemypower / totalenemypower)) + '%';
        ...
        document.getElementById('boss_health_text').innerHTML =
            'enemy military: ' + shorten(Math.max(0, enemypower)) + '/' + shorten(totalenemypower);
    } else {
    ...
    document.getElementById('pop').innerHTML =
        ...
        shorten(currentpop) +
        '/' +
        (currentpop > population && difficulty > 10
            ? "<strong class = 'color-r'>" + shorten(population) + '</strong>'
            : shorten(population));
    document.getElementById('food').innerHTML =
        'Food: ' +
        (food < currentpop
            ? "<strong class = 'color-r'>&nbsp;" + shorten(food) + '</strong>'
            : shorten(food));
    ...
    document.getElementById('resources').innerHTML = `Resources: ${shorten(resources)} (${
        resourcesgained >= 0 ? '+' : ''
    }${shorten(resourcesgained)})`;
}
function attack(power) {
    ...
    if (enemy_power > military) {
        ...
        information[0].choosetext(
            Math.floor(m_personnel / 4) + newcasual,
